...
class FullyConnectedLayer(Layer):
    ...
    def setAdamParams(self, value1, value2):
        self.__p1 = value1
        self.__p2 = value2
...
class ReLuLayer(Layer):
    ...
    def gradient(self):
        ...
        shape = np.shape(self.getPrevIn())
        gradientMatrix = np.zeros(shape)

        previousMatrix = self.getPrevIn()
        for i in range(0, shape[0]):
            for j in range(0, shape[1]):
                if previousMatrix[i, j] >= 0:
                    gradientMatrix[i, j] = 1
...
class SigmoidLayer(Layer):
    ...
    def gradient(self):
        ...
        for i in range(0, shape[0]):
            for j in range(0, shape[1]):
                gzj = self.getPrevOut()[i, j]
                gradientMatrix[i, j] = gzj * (1-gzj) + epsilon
...
class SoftmaxLayer(Layer):
    ...
    def forward(self, dataIn):
        ...
        sumA = np.sum( np.exp(dataIn - np.max(dataIn, axis=1)), axis=1) #Sum over all inputs for an observation
        Y = np.exp(dataIn - np.max(dataIn, axis=1)) / sumA
    ...
    def gradient(self):
        ...
        for o in range(0, numObservations):
            ...
            for i in range(0, size):
                gzi = self.getPrevOut()[o, i]
                for j in range(0, size):
                    gzj = self.getPrevOut()[o, j]
    ...
    def gradient(self):
        ...
        for i in range(0, shape[0]):
            for j in range(0, shape[1]):
                ...
                gradientMatrix[i, j] = (1-(gzj*gzj)) + epsilon
...
class Network:
    ...
    def getObjectiveFunction(self):
        return self.__objectiveFunction
